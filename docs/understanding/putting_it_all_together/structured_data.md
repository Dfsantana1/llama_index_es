...
```python
from llama_index_es import SQLDatabase
...
```python
from llama_index_es.indices.struct_store import NLSQLTableQueryEngine
...
```python
from llama_index_es.objects import (
    SQLTableNodeMapping,
    ObjectIndex,
    SQLTableSchema,
)
...
```python
from llama_index_es.indices.struct_store import SQLTableRetrieverQueryEngine
