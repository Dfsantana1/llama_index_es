...
from llama_index_es.embeddings import (
...
def test_resolve_embed_model(monkeypatch: MonkeyPatch) -> None:
    monkeypatch.setattr(
        "llama_index_es.embeddings.huggingface.HuggingFaceEmbedding.__init__",
        mock_hf_embeddings,
    )
    monkeypatch.setattr(
        "llama_index_es.embeddings.OpenAIEmbedding.__init__", mock_openai_embeddings
    )
