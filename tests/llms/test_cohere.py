...
def test_completion_model_basic(monkeypatch: MonkeyPatch) -> None:
    monkeypatch.setattr(
        "llama_index_es.llms.cohere.completion_with_retry", mock_completion_with_retry
    )
    ...
    monkeypatch.setattr(
        "llama_index_es.llms.cohere.completion_with_retry", mock_chat_with_retry
    )
...
async def test_async(monkeypatch: MonkeyPatch) -> None:
    mock_api_key = "fake_key"
    monkeypatch.setattr(
        "llama_index_es.llms.cohere.acompletion_with_retry", mock_acompletion_with_retry
    )
    llm = Cohere(model="command", api_key=mock_api_key)
    test_prompt = "test prompt"
    ...
    monkeypatch.setattr(
        "llama_index_es.llms.cohere.acompletion_with_retry", mock_achat_with_retry
    )
    message = ChatMessage(role="user", content=test_prompt)
