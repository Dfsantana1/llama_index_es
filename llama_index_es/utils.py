...
class GlobalsHelper:
    ...
    def stopwords(self) -> List[str]:
        ...
        if self._stopwords is None:
            ...
            from llama_index_es.utils import get_cache_dir
...
def set_global_tokenizer(tokenizer: Union[Tokenizer, Callable[[str], list]]) -> None:
    ...
    if isinstance(tokenizer, Tokenizer):
        llama_index_es.global_tokenizer = tokenizer.encode
    else:
        llama_index_es.global_tokenizer = tokenizer


def get_tokenizer() -> Callable[[str], List]:
    import llama_index_es

    if llama_index_es.global_tokenizer is None:
        tiktoken_import_err = (
            "`tiktoken` package not found, please run `pip install tiktoken`"
        )
        try:
    ...
    assert llama_index_es.global_tokenizer is not None
    return llama_index_es.global_tokenizer
...
def get_cache_dir() -> str:
    """Locate a platform-appropriate cache directory for llama_index_es,
    and create it if it doesn't yet exist.
    """
    # User override
    if "llama_index_es_CACHE_DIR" in os.environ:
        path = Path(os.environ["llama_index_es_CACHE_DIR"])

    # Linux, Unix, AIX, etc.
    elif os.name == "posix" and sys.platform != "darwin":
        path = Path("/tmp/llama_index_es")

    # Mac OS
    elif sys.platform == "darwin":
        path = Path(os.path.expanduser("~"), "Library/Caches/llama_index_es")
    ...
    # Windows (hopefully)
        ...
        path = Path(local, "llama_index_es")

    if not os.path.exists(path):
        os.makedirs(
            path, exist_ok=True
        )  # prevents https://github.com/jerryjliu/llama_index_es/issues/7362
    return str(path)
...
# Sample text from llama_index_es's readme
SAMPLE_TEXT = """
Context
...
def get_color_mapping(
    items: List[str], use_llama_index_es_colors: bool = True
) -> Dict[str, str]:
    """
    ...
    Args:
        ...
        use_llama_index_es_colors (bool, optional): Flag to indicate
    ...
    """
    if use_llama_index_es_colors:
        color_palette = _llama_index_es_COLORS
    else:
...
def _get_colored_text(text: str, color: str) -> str:
    ...
    """
    all_colors = {**_llama_index_es_COLORS, **_ANSI_COLORS}
